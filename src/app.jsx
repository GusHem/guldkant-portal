...
const API = {
    getQuotes: async () => {
        ...
        console.error("API did not return the expected structure { quotes: [...] }:", data);
        return [];
    },
...
const EditModal = ({ quote, isOpen, onClose, saveMutation, sendProposalMutation }) => {
    ...
    const handleSendProposal = () => { const quoteToSend = { ...formData, status: 'förslag-skickat', events: [...(formData.events || []), { timestamp: new Date().toISOString(), event: 'Förslag skickat till kund (via portal)' }] }; saveMutation.mutate(quoteToSend, { onSuccess: (data) => { if(data.id) sendProposalMutation.mutate(data.id); } }); };
    const handleApproveProposal = () => { const quoteToApprove = { ...formData, status: 'godkänd', events: [...(formData.events || []), { timestamp: new Date().toISOString(), event: 'Förslag godkänt av administratör' }] }; saveMutation.mutate(quoteToApprove); };
    const handleExportPdf = async () => { /* ... oförändrad PDF-logik ... */ };
    
    if (!isOpen || !formData) return null;
    const isSaving = saveMutation.isPending;
    const isNewQuote = !formData.id || formData.id.toString().startsWith('temp_');
...
function App() {
    ...
    const saveQuoteMutation = useMutation({
        ...
        onSuccess: () => {
            ...
            showToast(`Ärendet har sparats.`, 'success');
            handleCloseModal();
        },
        onError: (err) => {
    ...
    const sortedAndFilteredQuotes = useMemo(() => {
        ...
        const processedQuotes = rawQuotes.map(q => ({...q, total: calculateTotal(q)}));
        ...
        return processedQuotes.filter(q => {
            ...
            const searchMatch = term === '' || q.customer?.toLowerCase().includes(term) || q.id?.toString().toLowerCase().includes(term);
